...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        channel = x.shape[-1]
        x = F.linear(x, self.fc1.weight[:self.ratio * channel, :channel], self.fc1.bias[:self.ratio * channel])
        x = self.act(x)
        x = self.drop(x)
        x = F.linear(x, self.fc2.weight[:channel, :self.ratio * channel], self.fc2.bias[:channel])
        x = self.drop(x)
        return x
...
#        pdb.set_trace()
        ...
        qkv = F.linear(x, self.qkv.weight[:3 * C, :C], self.qkv.bias[:3 * C]).reshape(B, N, 3, num_heads, C // num_heads).permute(2, 0, 3, 1, 4)
#        qkv = self.qkv(x).reshape(B, N, 3, num_heads, C // num_heads).permute(2, 0, 3, 1, 4)
        ...
        x = F.linear(x, self.proj.weight[:C, :C], self.proj.bias[:C])
        x = self.proj_drop(x)
        return x
...
#        pdb.set_trace()
        channel = x.shape[-1]
        residual =  x
        x = F.layer_norm(x, (channel,), self.norm1.weight[:channel], self.norm1.bias[:channel], self.norm1.eps)
        x = residual + self.drop_path(self.attn(x))
        residual =  x
        x = F.layer_norm(x, (channel,), self.norm2.weight[:channel], self.norm2.bias[:channel], self.norm2.eps)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, resolution, heads, depth):
#        pdb.set_trace()
        B = x.shape[0]
        x = self.patch_embed(x)
        pos_embed = self.pos_embed
        if int(resolution / 16) != 14:
           x = torch.nn.functional.interpolate(x, size=(int(resolution / 16), int(resolution / 16)), mode='bilinear')
           pos_embed = torch.nn.functional.interpolate(self.pos_embed[:, 1:, :].reshape(1, 14, 14, -1).permute(0, 3, 1, 2), size=(int(resolution / 16), int(resolution / 16)), mode='bilinear')
           pos_embed = pos_embed.flatten(2).transpose(1, 2)
           pos_embed = torch.cat([self.pos_embed[0, 0, :].unsqueeze(0).unsqueeze(0), pos_embed], dim=1) 
        x = x.flatten(2).transpose(1, 2)
        channel = int(self.num_features * heads / self.num_heads)
        ...
        for blk in self.blocks[:depth]:
            x = blk(x)
#        pdb.set_trace()
        x = F.layer_norm(x, (channel,), self.norm.weight[:channel], self.norm.bias[:channel], self.norm.eps)
        return x[:, 0]

    def forward(self, x, resolution, heads, depth):
        x = self.forward_features(x, resolution, heads, depth)
        channel = x.shape[-1]
        x = F.linear(x, self.head.weight[:, :channel], self.head.bias)
        return x
